...
SPARSE_GNAT_CPP_ABI = "2026-05-15-mor-sparse-gnat-v1"
...
def module() -> Any:
    ...
    cache = _cache_dir()
    modname = f"_pycutfem_mor_sparse_gnat_{SPARSE_GNAT_CPP_ABI.replace('-', '_')}"
    built = cache / f"{modname}{_ext_suffix()}"
    lock = cache / f"{modname}.lock"
    src = Path(__file__).with_name("sparse_gnat_module.cpp")
