...
DEIM_ONLINE_CPP_ABI = "2026-05-15-mor-deim-online-v1"
...
def module() -> Any:
    ...
    cache = _cache_dir()
    modname = f"_pycutfem_mor_deim_online_{DEIM_ONLINE_CPP_ABI.replace('-', '_')}"
    built = cache / f"{modname}{_ext_suffix()}"
    lock = cache / f"{modname}.lock"
    src = Path(__file__).with_name("deim_online_module.cpp")
