...
GAUSS_NEWTON_CPP_ABI = "2026-05-15-mor-gauss-newton-v1"
...
def module() -> Any:
    ...
    cache = _cache_dir()
    modname = f"_pycutfem_mor_gauss_newton_{GAUSS_NEWTON_CPP_ABI.replace('-', '_')}"
    built = cache / f"{modname}{_ext_suffix()}"
    lock = cache / f"{modname}.lock"
    src = Path(__file__).with_name("gauss_newton_module.cpp")
