...
NATIVE_REDUCED_ASSEMBLER_CPP_ABI = "2026-05-15-mor-native-reduced-assembler-v1"
...
def module() -> Any:
    ...
    cache = _cache_dir()
    modname = f"_pycutfem_mor_native_reduced_assembler_{NATIVE_REDUCED_ASSEMBLER_CPP_ABI.replace('-', '_')}"
    built = cache / f"{modname}{_ext_suffix()}"
    lock = cache / f"{modname}.lock"
    src = Path(__file__).with_name("native_reduced_assembler_module.cpp")
