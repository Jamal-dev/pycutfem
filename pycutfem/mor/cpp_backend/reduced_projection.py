...
REDUCED_PROJECTION_CPP_ABI = "2026-05-15-mor-reduced-projection-v1"
...
def module() -> Any:
    ...
    cache = _cache_dir()
    modname = f"_pycutfem_mor_reduced_projection_{REDUCED_PROJECTION_CPP_ABI.replace('-', '_')}"
    built = cache / f"{modname}{_ext_suffix()}"
    lock = cache / f"{modname}.lock"
    src = Path(__file__).with_name("reduced_projection_module.cpp")
