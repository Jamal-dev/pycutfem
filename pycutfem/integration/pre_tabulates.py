...
def _tabulate_p1(xi, eta, N, dN):
    ...
    If Numba is available we JIT-compile this kernel for speed; otherwise this
    is a pure-Python implementation.
    """
    nE, nQ = xi.shape
    for e in _nb.prange(nE) if _HAVE_NUMBA else range(nE):
        for q in range(nQ):
            s = xi[e, q]; t = eta[e, q]
...
if _HAVE_NUMBA:
    _tabulate_p1 = _nb.njit(cache=True, fastmath=True, parallel=True)(_tabulate_p1)
    _tabulate_q1 = _nb.njit(cache=True, fastmath=True, parallel=True)(_tabulate_q1)
    _tabulate_q2 = _nb.njit(cache=True, fastmath=True, parallel=True)(_tabulate_q2)
...
if _HAVE_NUMBA:
    _q1_shape_grad = _nb.njit(cache=True, fastmath=True)(_q1_shape_grad)
    _q2_shape_grad = _nb.njit(cache=True, fastmath=True)(_q2_shape_grad)
