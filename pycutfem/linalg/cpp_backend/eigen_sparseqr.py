...
EIGEN_SPARSEQR_CPP_ABI = "2026-04-22-eigen-sparseqr-v1"
...
def module() -> Any:
    ...
    cache = _cache_dir()
    modname = f"_pycutfem_cpp_eigen_sparseqr_{EIGEN_SPARSEQR_CPP_ABI.replace('-', '_')}"
    built = cache / f"{modname}{_ext_suffix()}"
    lock = cache / f"{modname}.lock"
    src = Path(__file__).with_name("eigen_sparseqr_module.cpp")
