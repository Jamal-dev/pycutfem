...
STRUCTURAL_MESH_MOTION_STRATEGY_CPP_ABI = "2026-04-26-v3"
...
def module() -> Any:
    ...
    cache = _cache_dir()
    modname = (
        "_pycutfem_cpp_structural_mesh_motion_strategy_"
        f"{STRUCTURAL_MESH_MOTION_STRATEGY_CPP_ABI.replace('-', '_')}"
    )
    built = cache / f"{modname}{_ext_suffix()}"
    lock = cache / f"{modname}.lock"
    src = Path(__file__).with_name("structural_mesh_motion_strategy_module.cpp")
