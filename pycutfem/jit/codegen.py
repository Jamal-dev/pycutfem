...
class NumbaCodeGen:
        ...
        for op in ir_sequence:
            ...
            # --- LOAD OPERATIONS ---
                ...
                # ------------------------------------------------------------------
                    ...
                    base = f"u_{operand.name}"
                    coeff_sym = (
                        f"{base}_pos_loc" if operand.side == "+"
                        else f"{base}_neg_loc" if operand.side == "-"
                        else f"{base}_loc"
                    )
            ...
            # --- UNARY OPERATORS ---
                ...
                if a.role in ('test', 'trial'):
                    ...
                    for i, fname in enumerate(a.field_names):
                        ...
                        # body_lines.append(f"{grad_val_comp} = {phys_grad_basis}.T.copy() @ u_{fname}_loc")
            ...
            # ------------------------------------------------------------------
                ...
                # ---------------------------------------------------------------------
                 ...
                 # ------------ PRODUCT ---------------
                 # -------------------------------------
                #  print(f"BinaryOp: {op.op_symbol} with a.role={a.role}, b.role={b.role}, a.shape={a.shape}, b.shape={b.shape}, is_vector: {a.is_vector}/{b.is_vector}, is_gradient: {a.is_gradient}/{b.is_gradient}")
                 if op.op_symbol == '*':
    ...
    def _build_kernel_string(
        ...
        """
        # Each entry in solution_func_names already *is* the correct symbol
        # (e.g.  u_u_loc, u_u_pos_loc, …).  Pass it through verbatim.
        ...
        # ]
        param_order = [
            #  "gdofs_map", "node_coords",
             "qp_phys", "qw", "detJ", "J_inv",
             "normals", "phis",
             *sorted(required_args)
         ]
        ...
        # Slice the per-element block once per coefficient array
        coeffs_unpack_block = "\n".join(
            f"        {name}_e = {name}[e]"
            for name in sorted(solution_func_names)
        )
        ...
        # )
        # basis_unpack_block = "\n".join(
        #     f"""            # pick value at this quadrature point
        #             {arg}_q = {arg}[e, q] if {arg}.shape[0] == num_elements else {arg}[q]"""
        #     for arg in sorted(list(required_args))
        #     if arg.startswith(('b_', 'd_', 'g_'))
        # )
        basis_unpack_block = "\n".join(
...
""".lstrip()
        ...
        # print('*'*50)
        # print(f"Generate kernel: {final_kernel_src}")
        # print('*'*50)
