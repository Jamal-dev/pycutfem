...
CODEGEN_ABI = "2026-01-07-const-scalars-item-ana-tmp"
...
class KernelCache:
    ...
    def _import_with_fallback(self, modname: str, path: Path,
        ...
        try:
            ...
            # Stale modules may reference helpers that no longer exist.
            print(f"Kernel '{modname}' failed to import ({exc}). Rebuilding.")
            try:
                path.unlink()
            except OSError:
                pass
        ...
        if not hasattr(module, "PARAM_ORDER"):
            ...
            self._compile_and_write(path, ir_seq, codegen)
            module = self._import_module(modname, path)
        elif getattr(module, "CODEGEN_ABI", None) != CODEGEN_ABI:
            print(f"Kernel '{modname}' built with outdated ABI – rebuilding.")
            try:
                path.unlink()
            except OSError:
                pass
