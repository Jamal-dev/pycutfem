...
def _build_problem(*, nx: int = 2, ny: int = 2, q: int = 5):
    ...
    X_k.nodal_values[:] = np.clip(0.1 + 0.05 * rng.standard_normal(X_k.nodal_values.shape), 0.0, 1.0)
    X_n.nodal_values[:] = np.clip(0.1 + 0.05 * rng.standard_normal(X_n.nodal_values.shape), 0.0, 1.0)
...
def test_biofilm_one_domain_detached_mass_jacobian_fd_consistency():
    """
    FD check for the extended (v,p,u,phi,alpha,S,X) one-domain biofilm Jacobian.
    """
    dh, forms, field_to_func_k = _build_problem(nx=2, ny=2, q=3)
    ...
    def assemble_residual():
        _, R = assemble_form(Equation(None, forms.residual_form), dof_handler=dh, bcs=[], quad_order=3, backend="python")
    ...
    eps = 1.0e-8
    for gdof in probes:
        fld, _ = dh._dof_to_node_map[gdof]
        func = field_to_func_k[fld]
        old = float(func.get_nodal_values(np.asarray([gdof], dtype=int))[0])

        func.set_nodal_values(np.asarray([gdof], dtype=int), np.asarray([old + eps], dtype=float))
        R1 = assemble_residual()
        func.set_nodal_values(np.asarray([gdof], dtype=int), np.asarray([old], dtype=float))

        fd = (R1 - R0) / eps
        col = np.asarray(K.getcol(gdof).toarray()).reshape(-1)
        ...
        assert rel < 2.0e-6, f"FD mismatch at dof {gdof} ({fld}): rel={rel:.2e}"
