...
@pytest.mark.skipif(not _have_cpp_backend(), reason="cpp backend requires pybind11")
def test_loaded_native_reduced_artifact_solves_bound_constrained_problem(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("PYCUTFEM_CACHE_DIR", str(tmp_path / "mor_cross_cubic_constrained_artifact"))
    _mesh, dh, residual_runner, residual_args, tangent_runner, tangent_args = _cubic_native_pair()
    n_dofs = int(dh.total_dofs)
    artifact = NativeReducedArtifact(
        problem_id="cubic_reaction_constrained_smoke",
        trial_basis=np.ones((n_dofs, 1), dtype=float),
        offset=np.zeros(n_dofs, dtype=float),
        residual_kernel=NativeKernelReference(
            kernel_id="cubic_residual",
        ...
        bound_constraints=BoundConstraintSpec(
            ...
            upper=np.full(n_dofs, 1.5, dtype=float),
        ),
        solver_options={"max_iterations": 20, "residual_tol": 1.0e-14, "line_search": True, "adaptive_damping": True},
        metadata={"coefficient_arg_names": ("u_uh_loc",)},
    )
    path = tmp_path / "cubic_native_constrained_artifact.npz"
    artifact.save(path)
    loaded = load_native_reduced_artifact(path)
    runtime = loaded.instantiate(
        residual_metadata_capsule=native_kernel_metadata_from_runner(residual_runner),
    ...
    result = runtime.solve(np.array([1.2], dtype=float), constraint_method="pdas")

    assert result.converged
    assert result.backend == "cpp_native_pdas_online"
    np.testing.assert_allclose(result.coefficients, np.array([1.5]), rtol=1.0e-10, atol=1.0e-10)
    assert float(result.timing_counters["bound_violation"]) <= 1.0e-10


@pytest.mark.skipif(not _have_cpp_backend(), reason="cpp backend requires pybind11")
def test_loaded_native_reduced_artifact_dispatches_bound_constrained_deim(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("PYCUTFEM_CACHE_DIR", str(tmp_path / "mor_cross_cubic_constrained_deim_artifact"))
    _mesh, dh, residual_runner, residual_args, tangent_runner, tangent_args = _cubic_native_pair()
    n_dofs = int(dh.total_dofs)
    ...
    artifact = NativeReducedArtifact(
        problem_id="cubic_reaction_deim_constrained_smoke",
        trial_basis=np.ones((n_dofs, 1), dtype=float),
        offset=np.zeros(n_dofs, dtype=float),
        residual_kernel=NativeKernelReference(
            kernel_id="cubic_residual",
    ...
    path = tmp_path / "cubic_native_deim_constrained_artifact.npz"
    artifact.save(path)
    loaded = load_native_reduced_artifact(path)
    runtime = loaded.instantiate(
        residual_metadata_capsule=native_kernel_metadata_from_runner(residual_runner),
    ...
    assert result.converged
    assert result.backend == "cpp_native_deim_pdas_online"
    np.testing.assert_allclose(result.coefficients, np.array([1.5]), rtol=1.0e-10, atol=1.0e-10)
