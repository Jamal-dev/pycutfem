...
def test_named_kratos_validation_case_manifest_is_explicit():
    ...
    assert "four_point_shear" in blocked
...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "fluid_pumping_2D final first-step parity still needs the remaining "
        "Kratos Poromechanics nonlinear scheme-state sequencing; local FIC, "
        "nonlinear interface, interface flux, and first global Newton increment "
        "now have parity gates"
    ),
)
