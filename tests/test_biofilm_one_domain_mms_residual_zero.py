...
def test_biofilm_one_domain_mms_residual_zero_affine(backend):
    ...
    for field, fn in (
        ...
        if field.startswith("v_"):
            ...
            comp = 0 if field.endswith("x") else 1
            _set_field(dh, field, vals, func=prob["vS_k"].components[comp])
        else:
            comp = 0 if field.endswith("x") else 1
    ...
    # transport/reaction split used by the driver forms. On this coarse affine
    # MMS setup the assembled residual stays backend-consistent and bounded, but
    # it is no longer near machine zero.
    assert res < 1.0e-1
