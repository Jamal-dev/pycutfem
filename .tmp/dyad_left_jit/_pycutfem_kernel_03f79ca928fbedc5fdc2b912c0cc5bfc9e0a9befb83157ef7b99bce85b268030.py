...
PARAM_ORDER = ['gdofs_map', 'node_coords', 'qp_phys', 'qw', 'detJ', 'J_inv', 'normals', 'b_ux', 'b_uy', 'jit_const_0']
@numba.njit(parallel=True, fastmath=True, cache=True)
def kernel(
        gdofs_map, node_coords, qp_phys, qw, detJ, J_inv, normals, b_ux, b_uy, jit_const_0
    ):
    ...
    for e in numba.prange(num_elements):
        ...
        for q in range(qw.shape[1]):
            ...
            basis_stack_3 = np.stack((b_ux[e, q], b_uy[e, q]))
            # Outer: basis vector ⊗ basis vector -> mixed tensor
            outer_4 = basis_vector_outer_basis_vector(basis_stack_2, basis_stack_3, True, np.float64)
            # Dot: generic contraction (last axis of A · first axis of B)
            dot_5 = contract_last_first(const_np_arr_1, outer_4, np.float64)
