...
CODEGEN_ABI = "2026-04-08-abi-quadrature-state-v15"
...
PARAM_ORDER = ['gdofs_map', 'node_coords', 'qp_phys', 'qw', 'detJ', 'J_inv', 'normals', 'b_ux', 'b_uy', 'g_ux', 'g_uy', 'jit_const_0', 'u_u_k_loc']
@numba.njit(parallel=True, fastmath=True, cache=True)
def kernel(
        gdofs_map, node_coords, qp_phys, qw, detJ, J_inv, normals, b_ux, b_uy, g_ux, g_uy, jit_const_0, u_u_k_loc
    ):
    ...
    for e in numba.prange(num_elements):
        ...
        for q in range(qw.shape[1]):
            ...
            g_ux_q = g_ux[e,q]
            g_uy_q = g_uy[e,q]
            u_k_val_1 = np.zeros((2,), dtype=np.float64)
            val_comp_2 = load_variable_qp(u_u_k_loc_e, b_ux[e, q])
            u_k_val_1[0] = val_comp_2
            val_comp_3 = load_variable_qp(u_u_k_loc_e, b_uy[e, q])
            u_k_val_1[1] = val_comp_3
            basis_stack_4 = np.stack((b_ux[e, q], b_uy[e, q]))
            # Outer: value vector ⊗ basis vector -> basis tensor
            outer_5 = value_vector_outer_basis_vector(u_k_val_1, basis_stack_4, np.float64)
            const_np_arr_6 = jit_const_0
            # Dot: generic contraction (last axis of A · first axis of B)
            dot_7 = contract_last_first(outer_5, const_np_arr_6, np.float64)
            J_inv_q = J_inv[e, q]
            grad_loc_8 = np.ascontiguousarray(g_ux[e, q]) @ np.ascontiguousarray(J_inv_q.copy())
            grad_loc_9 = np.ascontiguousarray(g_uy[e, q]) @ np.ascontiguousarray(J_inv_q.copy())
            grad_stack_10 = np.stack((grad_loc_8, grad_loc_9))
            # Inner(LHS): orient rows=test, cols=trial
            # Inner(Vector, Vector): dot product, LHS mass matrix
            inner_11 = dot_mass_test_trial(grad_stack_10, dot_7, np.float64)
            Ke += inner_11 * w_q
