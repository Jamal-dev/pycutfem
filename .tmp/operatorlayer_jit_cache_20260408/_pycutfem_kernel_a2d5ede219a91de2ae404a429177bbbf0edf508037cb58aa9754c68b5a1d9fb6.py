...
PARAM_ORDER = ['gdofs_map', 'node_coords', 'qp_phys', 'qw', 'detJ', 'J_inv', 'normals', 'b_ux', 'b_uy', 'example2_local_dvms_old_subscale_velocity', 'example2_local_inv_dt', 'example2_local_rho_f', 'g_mx', 'g_my', 'jit_const_0', 'owner_id', 'u_d_mesh_loc']
@numba.njit(parallel=True, fastmath=True, cache=True)
def kernel(
        gdofs_map, node_coords, qp_phys, qw, detJ, J_inv, normals, b_ux, b_uy, example2_local_dvms_old_subscale_velocity, example2_local_inv_dt, example2_local_rho_f, g_mx, g_my, jit_const_0, owner_id, u_d_mesh_loc
    ):
    ...
    for e in numba.prange(num_elements):
        ...
        for q in range(qw.shape[1]):
            ...
            const_val_10 = example2_local_inv_dt.item()
            # Product: const * const
            # Product: scalar * scalar/np.ndarry → scalar/np.ndarray
            res_11 = const_val_9 * const_val_10
            ewc_12 = example2_local_dvms_old_subscale_velocity[owner_id[e]]
            # Product: const * value
            # Product: scalar * Vector/Tensor → Vector/Tensor
            res_13 = mul_scalar(res_11, ewc_12, np.float64)
            basis_stack_14 = np.stack((b_ux[e, q], b_uy[e, q]))
            # Dot plan: value vector · semantic basis rank-1
            dot_15 = basis_dot_const_vector(basis_stack_14, res_13, np.float64)
            # Product: value * test
            # Product: scalar * Vector/Tensor → Vector/Tensor
            res_16 = mul_scalar(det2_8, dot_15[0], np.float64)
            Fe += res_16 * w_q
