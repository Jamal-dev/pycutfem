...
PARAM_ORDER = ['gdofs_map', 'node_coords', 'qp_phys', 'qw', 'detJ', 'J_inv', 'normals', 'b_p', 'g_mx', 'g_my', 'g_ux', 'g_uy', 'jit_const_0', 'u_d_mesh_loc']
@numba.njit(parallel=True, fastmath=True, cache=True)
def kernel(
        gdofs_map, node_coords, qp_phys, qw, detJ, J_inv, normals, b_p, g_mx, g_my, g_ux, g_uy, jit_const_0, u_d_mesh_loc
    ):
    ...
    for e in numba.prange(num_elements):
        ...
        for q in range(qw.shape[1]):
            ...
            inner_12 = inner_grad_function_grad_test(cof2_8, grad_stack_11, np.float64)
            basis_reshaped_13 = b_p[e, q][np.newaxis, :].copy()
            # Product: trial * test
            # Product: scalar Test × scalar Trial → mixed scalar basis tensor
            res_14 = dot_mass_test_trial(basis_reshaped_13, inner_12, np.float64)
            Ke += res_14 * w_q
