...
PARAM_ORDER = ['gdofs_map', 'node_coords', 'qp_phys', 'qw', 'detJ', 'J_inv', 'normals', 'b_p', 'example2_local_pressure_gauge', 'u_p_k_loc']
@numba.njit(parallel=True, fastmath=True, cache=True)
def kernel(
        gdofs_map, node_coords, qp_phys, qw, detJ, J_inv, normals, b_p, example2_local_pressure_gauge, u_p_k_loc
    ):
    num_elements        = qp_phys.shape[0]
    # n_dofs_per_element  = 3
    ...
    for e in numba.prange(num_elements):
        ...
        for q in range(qw.shape[1]):
            ...
            J_inv_q = J_inv[e,q]
            b_p_q = b_p[e,q]
            const_val_1 = example2_local_pressure_gauge.item()
            p_k_val_2 = load_variable_qp(u_p_k_loc_e, b_p[e, q])
            # Product: const * value
            # Product: scalar * scalar/np.ndarry → scalar/np.ndarray
            res_3 = const_val_1 * p_k_val_2
            basis_reshaped_4 = b_p[e, q][np.newaxis, :].copy()
            # Product: value * test
            # Product: scalar * Vector/Tensor → Vector/Tensor
            res_5 = mul_scalar(res_3, basis_reshaped_4[0], np.float64)
            Fe += res_5 * w_q
