...
PARAM_ORDER = ['gdofs_map', 'node_coords', 'qp_phys', 'qw', 'detJ', 'J_inv', 'normals', 'b_mx', 'b_my', 'example2_local_zero', 'example2_local_zero_vec']
@numba.njit(parallel=True, fastmath=True, cache=True)
def kernel(
        gdofs_map, node_coords, qp_phys, qw, detJ, J_inv, normals, b_mx, b_my, example2_local_zero, example2_local_zero_vec
    ):
    ...
    for e in numba.prange(num_elements):
        ...
        for q in range(qw.shape[1]):
            ...
            b_mx_q = b_mx[e,q]
            b_my_q = b_my[e,q]
            const_val_1 = example2_local_zero.item()
            basis_stack_2 = np.stack((b_mx[e, q], b_my[e, q]))
            const_np_arr_3 = example2_local_zero_vec
            # Dot plan: semantic basis rank-1 · value vector
            dot_4 = basis_dot_const_vector(basis_stack_2, const_np_arr_3, np.float64)
            # Product: const * test
            # Product: scalar * Vector/Tensor → Vector/Tensor
            res_5 = mul_scalar(const_val_1, dot_4[0], np.float64)
            Fe += res_5 * w_q
