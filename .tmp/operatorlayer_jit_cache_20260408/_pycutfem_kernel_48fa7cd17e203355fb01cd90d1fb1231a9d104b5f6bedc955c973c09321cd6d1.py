...
PARAM_ORDER = ['gdofs_map', 'node_coords', 'qp_phys', 'qw', 'detJ', 'J_inv', 'normals', 'b_ux', 'b_uy', 'g_mx', 'g_my', 'jit_const_0', 'jit_const_1', 'u_d_mesh_loc']
@numba.njit(parallel=True, fastmath=True, cache=True)
def kernel(
        gdofs_map, node_coords, qp_phys, qw, detJ, J_inv, normals, b_ux, b_uy, g_mx, g_my, jit_const_0, jit_const_1, u_d_mesh_loc
    ):
    ...
    for e in numba.prange(num_elements):
        ...
        for q in range(qw.shape[1]):
            ...
            basis_stack_10 = np.stack((b_ux[e, q], b_uy[e, q]))
            # Product: const * trial
            # Product: scalar * Vector/Tensor → Vector/Tensor
            res_11 = mul_scalar(const_val_9, basis_stack_10, np.float64)
            basis_stack_12 = np.stack((b_ux[e, q], b_uy[e, q]))
            # Dot plan: basis-basis mass contraction
            dot_13 = dot_mass_test_trial(basis_stack_12, res_11, np.float64)
            # Product: value * mixed
            # Product: scalar * Vector/Tensor → Vector/Tensor
            res_14 = mul_scalar(det2_8, dot_13, np.float64)
            Ke += res_14 * w_q
