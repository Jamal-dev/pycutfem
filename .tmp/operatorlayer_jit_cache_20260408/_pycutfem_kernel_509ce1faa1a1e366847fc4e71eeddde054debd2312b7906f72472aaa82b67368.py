...
PARAM_ORDER = ['gdofs_map', 'node_coords', 'qp_phys', 'qw', 'detJ', 'J_inv', 'normals', 'ana_0', 'b_dx', 'b_dy', 'jit_const_0']
@numba.njit(parallel=True, fastmath=True, cache=False)
def kernel(
        gdofs_map, node_coords, qp_phys, qw, detJ, J_inv, normals, ana_0, b_dx, b_dy, jit_const_0
    ):
    ...
    for e in numba.prange(num_elements):
        ...
        for q in range(qw.shape[1]):
            ...
            ana_val_2 = ana_0[e, q]
            basis_stack_3 = np.stack((b_dx[e, q], b_dy[e, q]))
            # Dot plan: value vector · semantic basis rank-1
            dot_4 = basis_dot_const_vector(basis_stack_3, ana_val_2, np.float64)
            # Product: const * test
