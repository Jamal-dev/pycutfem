...
PARAM_ORDER = ['gdofs_map', 'node_coords', 'qp_phys', 'qw', 'detJ', 'J_inv', 'normals', 'b_p', 'jit_const_0']
@numba.njit(parallel=True, fastmath=True, cache=True)
def kernel(
        gdofs_map, node_coords, qp_phys, qw, detJ, J_inv, normals, b_p, jit_const_0
    ):
    ...
    for e in numba.prange(num_elements):
        ...
        for q in range(qw.shape[1]):
            ...
            basis_reshaped_2 = b_p[e, q][np.newaxis, :].copy()
            # Product: const * trial
            # Product: scalar * Vector/Tensor → Vector/Tensor
            res_3 = mul_scalar(const_val_1, basis_reshaped_2, np.float64)
            basis_reshaped_4 = b_p[e, q][np.newaxis, :].copy()
            # Product: trial * test
            # Product: scalar Test × scalar Trial → mixed scalar basis tensor
            res_5 = dot_mass_test_trial(basis_reshaped_4, res_3, np.float64)
            Ke += res_5 * w_q
