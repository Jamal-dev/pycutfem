...
def kernel(
    ...
    for e in numba.prange(num_elements):
        ...
        for q in range(qw.shape[1]):
            ...
            g_ux_q = g_ux[e,q]
            g_uy_q = g_uy[e,q]
            basis_stack_1 = np.stack((b_ux[e, q], b_uy[e, q]))
            J_inv_q = J_inv[e, q]
            ...
            dot_5 = dot_vec_grad_components(basis_stack_1, grad_stack_4, True, np.float64)
            const_np_arr_6 = jit_const_0
            # Dot: mixed basis (m,n,k) · constant vector → matrix
            dot_7 = dot_mixed_const(dot_5, const_np_arr_6, np.float64)
            Ke += dot_7 * w_q
