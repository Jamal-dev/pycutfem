#!/usr/bin/env python
"""
Fully-Eulerian FSI MMS (Variant B: sinusoidal "flapping beam" with interface forcing).

This variant exercises:
  - Solid kinematics:  u_s = ∂_t d  and  u_s · ∇d = 0
  - Interface coupling: traction terms including pressure/shear signs
  - C++ JIT backend parity on dInterface kernels (via full assembly)

Because the sinusoidal time dependence does NOT naturally match fluid/solid shear
tractions, we add an explicit interface forcing:
  g = t_f - t_s  (with a single normal n from solid(-) to fluid(+))

Run (example):
  PYCUTFEM_JIT_BACKEND=cpp conda run --no-capture-output -n fenicsx \
    python -u examples/turek_fsi_fully_eulerian/fsi_mms_flapping_beam.py
"""
...
def main() -> None:
    ...
    # Interface forcing: g = t_f - t_s (y-component for this shear setup).
    def g_ifc_y(x):
        t1 = float(time_state["t_curr"])
    ...
    residual_form = (
        ...
        # Distribute interface forcing to match the κ-weighted Nitsche consistency.
        + kappa_neg * dot(g_ifc, v_f_R) * dGamma
        + kappa_pos * dot(g_ifc, v_s_R) * dGamma
    )
    ...
    t0 = 0.0
    for (field, setter) in (
        ("u_pos_x", lambda xy: 0.0 * xy[:, 0]),
        ("u_pos_y", lambda xy: u_exact_y(xy[:, 0], t0)),
        ("vs_neg_x", lambda xy: 0.0 * xy[:, 0]),
        ("vs_neg_y", lambda xy: u_exact_y(xy[:, 0], t0)),
        ("d_neg_x", lambda xy: 0.0 * xy[:, 0]),
        ("d_neg_y", lambda xy: w_exact(xy[:, 0], t0)),
    ):
        gd = np.asarray(dh.get_field_slice(field), dtype=int)
        xy = dh.get_dof_coords(field)
        if field.startswith("u_"):
            target_n, target_k = uf_n, uf_k
        elif field.startswith("vs_"):
            target_n, target_k = us_n, us_k
        else:
            target_n, target_k = disp_n, disp_k
        vals = setter(xy)
        target_n.set_nodal_values(gd, vals)
        target_k.set_nodal_values(gd, vals)
    ...
    print(f"[mms-B] max|u_f_y - u_exact| = {e_u:.3e}")
    print(f"[mms-B] max|u_s_y - u_exact| = {e_us:.3e}")
    print(f"[mms-B] max|d_s_y - d_exact| = {e_d:.3e}")
