...
def solve_level(
    *,
    ...
    return {"h": float(h), "L2": errL2, "H1": errH1, "jump": float(jump), "flux": float(flux)}
...
def main() -> None:
    ...
    for lev in range(int(args.levels)):
        ...
        ny_pos = ny_neg + 1  # keep meshes non-matching on the interface
        rows.append(
            solve_level(
    ...
    print("\nNon-matching Poisson (Nitsche) MMS: k-jump, interface x=0.5")
    print(f"element={args.element} degree={args.degree} volume={args.backend_volume} interface={args.backend_interface}")
    print("\n  i       h           L2             rate      H1             rate      ||[u]||_G      ||flux||_G")
    for i, r in enumerate(rows):
        if i == 0:
            print(
                f"{i:3d}  {r['h']:.3e}  {r['L2']:.3e}    {'-':>6}  {r['H1']:.3e}    {'-':>6}  {r['jump']:.3e}  {r['flux']:.3e}"
            )
            continue
        r0 = rows[i - 1]
        print(
            f"{i:3d}  {r['h']:.3e}  {r['L2']:.3e}  {_rate(r0['h'], r0['L2'], r['h'], r['L2']):6.2f}  "
            f"{r['H1']:.3e}  {_rate(r0['h'], r0['H1'], r['h'], r['H1']):6.2f}  {r['jump']:.3e}  {r['flux']:.3e}"
        )
