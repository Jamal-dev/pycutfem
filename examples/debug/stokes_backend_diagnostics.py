...
def build_stokes_problem(with_deformation: bool) -> ProblemData:
    ...
    term_pos_vol1_restricted = (2 * mu[1] * inner(epsilon(vel_trial_pos), epsilon(vel_test_pos))) * dx_pos
    term_pos_vol2_restricted = (-div(vel_test_pos) * p_pos_R) * dx_pos
    term_pos_vol3_restricted = (-div(vel_trial_pos) * q_pos_R) * dx_pos
    lhs_terms.extend([term_pos_vol1_restricted, term_pos_vol2_restricted, term_pos_vol3_restricted])
    lhs_labels.extend([
        "vol_pos_restricted_elastic",
        "vol_pos_restricted_pressure_test",
        "vol_pos_restricted_pressure_trial",
    ])

    term_pos_vol1_full = (2 * mu[1] * inner(epsilon(vel_trial_pos), epsilon(vel_test_pos))) * dx_pos
    term_pos_vol2_full = (-div(vel_test_pos) * p_trial_pos) * dx_pos
    term_pos_vol3_full = (-div(vel_trial_pos) * q_test_pos) * dx_pos
    lhs_terms.extend([term_pos_vol1_full, term_pos_vol2_full, term_pos_vol3_full])
    lhs_labels.extend([
        "vol_pos_full_elastic",
        "vol_pos_full_pressure_test",
        "vol_pos_full_pressure_trial",
    ])

    term_neg_vol1_restricted = (2 * mu[0] * inner(epsilon(vel_trial_neg), epsilon(vel_test_neg))) * dx_neg
    term_neg_vol2_restricted = (-div(vel_test_neg) * p_neg_R) * dx_neg
    term_neg_vol3_restricted = (-div(vel_trial_neg) * q_neg_R) * dx_neg
    lhs_terms.extend([term_neg_vol1_restricted, term_neg_vol2_restricted, term_neg_vol3_restricted])
    lhs_labels.extend([
        "vol_neg_restricted_elastic",
        "vol_neg_restricted_pressure_test",
        "vol_neg_restricted_pressure_trial",
    ])

    term_neg_vol1_full = (2 * mu[0] * inner(epsilon(vel_trial_neg), epsilon(vel_test_neg))) * dx_neg
    term_neg_vol2_full = (-div(vel_test_neg) * p_trial_neg) * dx_neg
    term_neg_vol3_full = (-div(vel_trial_neg) * q_test_neg) * dx_neg
    lhs_terms.extend([term_neg_vol1_full, term_neg_vol2_full, term_neg_vol3_full])
    lhs_labels.extend([
        "vol_neg_full_elastic",
        "vol_neg_full_pressure_test",
        "vol_neg_full_pressure_trial",
    ])
...
def main():
    ...
    for part in ("lhs", "rhs"):
        ...
        print(
            f"  {part.upper()} terms: {len(entries)} entries,"
            f" worst max|Δ|={worst['max_abs']:.3e} (rel={worst['rel_fro']:.3e}) -> {worst['label']}"
        )
        if pd is not None:
            df = pd.DataFrame(entries)
            df = df.sort_values("max_abs", ascending=False).head(6)
            with pd.option_context("display.max_columns", None):
                print(df[["label", "measure", "max_abs", "rel_fro"]])
        else:
            top = sorted(entries, key=lambda e: e["max_abs"], reverse=True)[:6]
            for item in top:
                print(
                    f"    {item['label']:<35} max|Δ|={item['max_abs']:.3e} rel={item['rel_fro']:.3e}"
                )
