...
def main() -> None:
    ap = argparse.ArgumentParser(description="PyCutFEM scalar Newton parity run on a shared Gmsh mesh.")
    ap.add_argument(
        "--mesh-file",
        type=str,
    ...
    ap.add_argument("--newton-tol", type=float, default=1.0e-16)
    ap.add_argument("--max-it", type=int, default=50)
    ap.add_argument("--csv", type=str, default="examples/debug/out/parity/scalar_newton_pycutfem.csv")
    args = ap.parse_args()
    ...
    # Parity PDE: -Δu + u^3 = 1 on Ω, u=0 on ∂Ω.
    f = Constant(1.0)
    u3 = u_k * u_k * u_k
    residual = inner(grad(u_k), grad(u_test)) * dx_form + u3 * u_test * dx_form - f * u_test * dx_form

    jacobian = inner(grad(du), grad(u_test)) * dx_form + (Constant(3.0) * u_k * u_k) * du * u_test * dx_form
    ...
    solver = NewtonSolver(
        ...
        newton_params=NewtonParameters(
            newton_tol=float(args.newton_tol), max_newton_iter=int(args.max_it), ls_mode="dealii"
        ),
    ...
    if not converged:
        raise RuntimeError("Scalar Newton did not converge.")
    ...
    print(f"[scalar_newton_pycutfem] dofs={len(values)} u[max]={float(values.max()):.12e} wrote {out_csv}")
