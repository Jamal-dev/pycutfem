...
def _run_one_model(
    *,
    ...
    # Activate only v,p,alpha,(S).
    inactive = ["vS_x", "vS_y", "u_x", "u_y", "phi"]
    if not bool(args.solve_substrate):
        inactive.append("S")
    _mark_inactive_fields(dh, *inactive)
    ...
    def _append_row(*, t_k: float, step_no: int) -> None:
        ...
        if vtk_every > 0 and (step_no % vtk_every == 0 or abs(t_k - float(args.t_final)) < 1.0e-12):
            export_vtk(
    ...
    solver = PdasNewtonSolver(
        ...
        bcs=bcs,
        bcs_homog=bcs_homog,
        vi_params=VIParameters(c=float(args.vi_c), active_tol=float(args.vi_active_tol), project_initial_guess=True),
