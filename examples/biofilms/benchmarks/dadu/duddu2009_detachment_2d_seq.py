...
def _build_detachment_rate(
    *,
    ...
    if key == "shear":
        ...
        # Use `mu_tau` for the shear-stress scaling so we can compare against the
        # paper even when the flow is solved with a numerically convenient `mu_f`.
        tau = Constant(2.0) * mu_tau * tau_rate
