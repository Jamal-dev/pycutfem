...
def main() -> None:
    ...
    ap.add_argument("--exp-csv", type=str, default="out/_lie_exp_s1_dx/timeseries.csv")
    ...
    for i, c in enumerate(colors):
        ax.plot(t_exp, dx_exp_s[:, i] * 1.0e3, color=c, linewidth=2.0, label=f"exp line{i+1}")
        ax.plot(t_sim, dx_sim[:, i] * 1.0e3, color=c, linestyle="--", linewidth=2.0, label=f"sim line{i+1}")
    ax.set_xlabel("t [s]")
    ax.set_ylabel("dx [mm]")
    ax.grid(True, alpha=0.3)
    ax.legend(loc="best", ncols=2, fontsize=8, frameon=True)
    ax.set_title(f"Lie benchmark: exp vs sim (RMSE={rmse_tot*1e3:.3g} mm)")
    fig.tight_layout()

    if str(args.out).strip():
        out_png = Path(str(args.out))
    else:
        out_png = sim_csv.with_name("compare_exp_sim_dx.png")
