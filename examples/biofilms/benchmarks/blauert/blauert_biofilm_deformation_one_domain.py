...
def main() -> None:
    ...
    use_alpha_phi_vi_bounds = bool(transport_mode == "pde" and bool(getattr(args, "alpha_phi_vi_bounds", True)))
    ...
    if newton_solver_key == "snes":
        ...
        if use_alpha_phi_vi_bounds:
            missing = [name for name in ("alpha", "phi") if name not in set(getattr(dh, "field_names", []))]
            if missing:
                logging.warning(
                    "Requested --alpha-phi-vi-bounds but field(s) %s not found in DOF handler; skipping bounds.", missing
                )
            else:
                solver.set_box_bounds(by_field={"alpha": (0.0, 1.0), "phi": (0.0, 1.0)})
                logging.info("[setup] enabled SNES VI bounds: alpha,phi ∈ [0,1].")
    else:
        # PDAS / semismooth Newton for box constraints. This uses the same reduced assembly
        # pipeline as the Python Newton implementation (∞-norm based line-search), and
        # tends to be more reliable than SNES for tight per-DOF tolerances.
        ...
        if use_alpha_phi_vi_bounds:
            ...
            if missing:
                ...
                solver.set_box_bounds(by_field={"alpha": (0.0, 1.0), "phi": (0.0, 1.0)})
                logging.info("[setup] enabled PDAS VI bounds: alpha,phi ∈ [0,1].")
