...
# test_mixed_ghost_with_fenicsx.py
# Compare CutFEM ghost-penalty blocks for mixed fields (u_pos, u_neg, p_pos, p_neg)
# against equivalent FEniCSx assemblies on ghost_pos and ghost_neg facet sets.
#
# Blocks checked (no h-scaling by default; toggle --use-h):
#   POS: u_x, u_y, p  on ghost_pos
#   NEG: u_x, u_y, p  on ghost_neg
#
# Run examples:
#   python test_mixed_ghost_with_fenicsx.py --nx 6 --ny 4 --deg 4
#   python test_mixed_ghost_with_fenicsx.py --nx 10 --ny 8 --deg 6 --use-h
#
import argparse
...
from pycutfem.ufl.expressions import (
    TrialFunction, TestFunction, Constant, Jump, CellDiameter
)
...
def locate_ghost_pos_neg(mesh, levelset, tol=1e-12):
    """
    Replicate Mesh.classify_edges() for ghost_pos/ghost_neg:
      - interior facets only (two neighbors)
      - if exactly one neighbor is 'cut' and the other is 'outside' -> ghost_pos
      - if exactly one neighbor is 'cut' and the other is 'inside'  -> ghost_neg
      - if both neighbors are 'cut' -> ghost_both (ignored here)
      - interface edges (sign change on endpoints) are excluded
    """
    ...
    for f in range(f2c.num_nodes):
        ...
        if phi0 * phi1 < 0:  # interface edge -> exclude
            continue
        tags = []
        for c in cells:
            vs_c = c2v.links(c)
...
# --------------------------- Build both sides ---------------------------------
def setup_cut_mixed(nx=4, ny=2, order=1):
...
def setup_fenicsx_scalar(nx=4, ny=2, order=1):
...
def assemble_fx_on_facets(Q, p, q, mesh_fx, facets, deg, use_h=False):
    if facets.size == 0:
        return None
    ...
    form = ufl.jump(p) * ufl.jump(q) * dS(1)
    if use_h:
        form = (ufl.avg(ufl.CellDiameter(mesh_fx))**-2) * form
...
def main():
    ...
    # Exact bitsets like stokes_cut.py: only ghost_pos and ghost_neg (no interface or ghost_both)
    ...
    gamma_v = Constant(1.0)
    gamma_p = Constant(1.0)
    h = CellDiameter()
    ...
    scale_v = (gamma_v / (h*h)) if use_h else gamma_v
    scale_p = gamma_p

    A_pos_x = scale_v * Jump(upx) * Jump(vpx) * dG_pos
    A_pos_y = scale_v * Jump(upy) * Jump(vpy) * dG_pos
    A_pos_p = scale_p * Jump(ppos) * Jump(qpos) * dG_pos

    A_neg_x = scale_v * Jump(unx) * Jump(vnx) * dG_neg
    A_neg_y = scale_v * Jump(uny) * Jump(vny) * dG_neg
    A_neg_p = scale_p * Jump(pneg) * Jump(qneg) * dG_neg
    ...
    # FEniCSx side: one scalar space reused
    mesh_fx, Q_fx, p_fx, q_fx = setup_fenicsx_scalar(nx, ny, order)
    pos_facets, neg_facets = locate_ghost_pos_neg(mesh_fx, ls)

    A_fx_pos = assemble_fx_on_facets(Q_fx, p_fx, q_fx, mesh_fx, pos_facets, deg, use_h=use_h)
    A_fx_neg = assemble_fx_on_facets(Q_fx, p_fx, q_fx, mesh_fx, neg_facets, deg, use_h=use_h)
    ...
    print("\n=== Mixed ghost blocks: CutFEM vs FEniCSx ===")
