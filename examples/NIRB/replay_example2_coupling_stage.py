...
def main() -> None:
    ...
    exact_fluid_load_lookup = CoordinateLookup(
        np.asarray(post_update["fluid_load_coords_ref"], dtype=float),
        np.asarray(post_update["fluid_load_values"], dtype=float),
        dim=2,
    )
    exact_structure_load_lookup = _resample_lookup_to_coords(
        _negate_lookup(exact_fluid_load_lookup),
        solid_iface_coords,
    )
    ...
    exact_fluid_newton_tol = min(float(args.newton_tol), 1.0e-8)
    ...
    stage_solver = _get_or_create_cached_stage_solver(
        ...
        quad_order=int(args.quad_order),
        backend=str(args.backend),
        operators=[fluid_predictor_operator, fluid_exact_operator],
    ...
    next_guess_cmp = _compare_point_fields(
        ref_coords=np.asarray(post_update["fluid_load_coords_ref"], dtype=float),
        ref_values=np.asarray(post_update["fluid_load_values"], dtype=float),
        local_coords=np.asarray(exact_fluid_load_lookup.coords, dtype=float),
        local_values=np.asarray(exact_fluid_load_lookup.values, dtype=float),
    )
    ...
    if isinstance(full_stage_state, dict):
        ...
        summary["fluid_velocity_full_cmp"] = _compare_point_fields(
            ref_coords=np.asarray(full_stage_state["node_coords_ref"], dtype=float),
            ref_values=np.asarray(full_stage_state["velocity"], dtype=float),
        ...
        summary["fluid_pressure_full_cmp"] = _compare_point_fields(
            ref_coords=np.asarray(full_stage_state["node_coords_ref"], dtype=float),
            ref_values=np.asarray(full_stage_state["pressure"], dtype=float).reshape(-1, 1),
        ...
        summary["fluid_mesh_displacement_full_cmp"] = _compare_point_fields(
            ref_coords=np.asarray(full_stage_state["node_coords_ref"], dtype=float),
            ref_values=np.asarray(full_stage_state["mesh_displacement"], dtype=float),
