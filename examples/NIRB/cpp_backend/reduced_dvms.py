...
def module() -> Any:
    ...
    cache = _cache_dir()
    modname = f"_pycutfem_nirb_reduced_dvms_{REDUCED_DVMS_CPP_ABI.replace('-', '_')}"
    built = cache / f"{modname}{_ext_suffix()}"
    lock = cache / f"{modname}.lock"
    src = Path(__file__).with_name("reduced_dvms_module.cpp")
