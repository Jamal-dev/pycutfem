...
def _set_local_fields_from_dump(*, fluid: dict[str, object], eid: int, dump, dt: float) -> None:
    ...
    velocity_prev = np.asarray(dump["velocity_prev"], dtype=float).reshape(3, 2)
    acceleration = np.asarray(dump["acceleration"], dtype=float).reshape(3, 2)
...
def _set_dvms_state_from_dump(*, fluid: dict[str, object], eid: int, dump) -> None:
    ...
    advproj = np.asarray(dump["advproj"], dtype=float).reshape(3, 2)
    divproj = np.asarray(dump["divproj"], dtype=float).reshape(3)
...
def main() -> None:
    ...
    perm = _INTERLEAVED_TO_GROUPED_TRI3
    ...
    for name in ordered_lhs_terms:
        ...
        lhs_total += lhs_term
        lhs_blocks[name] = np.asarray(lhs_term, dtype=float)
    ...
    for name in ordered_rhs_terms:
        ...
        rhs_total += rhs_term
        rhs_blocks[name] = np.asarray(rhs_term, dtype=float)
