...
from examples.NIRB.dvms import _advance_fluid_dvms_history_after_step, _update_fluid_dvms_predicted_subscale, _update_fluid_dvms_state_from_previous_step
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare pycutfem DVMS predicted subscale against Kratos accepted-step quadrature state.")
    parser.add_argument(
        "--kratos-step-dir",
        type=Path,
...
def main() -> None:
    ...
    for step in range(1, target_step + 1):
        ...
        if step == target_step:
            ...
            local_subscale = np.asarray(fluid["dvms_state"].predicted_subscale_velocity, dtype=float)
            payload = {
                "step": int(step),
                "backend": str(args.backend),
