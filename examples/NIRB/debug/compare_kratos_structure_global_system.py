...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--linear-backend", choices=("scipy", "eigen_sparseqr", "petsc", "amgcl"), default="petsc")
    parser.add_argument("--quad-order", type=int, default=6)
    parser.add_argument(
        "--restart-from-checkpoint",
        type=Path,
        default=None,
        help="Optional local checkpoint payload to seed the actual previous-step structure state.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> None:
    ...
    report = {
        ...
        "quad_order": int(args.quad_order),
        "restart_from_checkpoint": None if args.restart_from_checkpoint is None else str(Path(args.restart_from_checkpoint).resolve()),
