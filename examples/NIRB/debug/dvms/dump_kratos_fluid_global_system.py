...
def main() -> None:
    ...
    try:
        ...
        elements = list(root_model_part.Elements)
        element_ids = np.asarray([int(elem.Id) for elem in elements], dtype=int)
        subscale_velocity_blocks: list[np.ndarray] = []
        subscale_pressure_blocks: list[np.ndarray] = []
        subscale_q_counts = np.zeros((len(elements),), dtype=int)
        for idx, elem in enumerate(elements):
            elem_subscale_velocity = np.asarray(
                elem.CalculateOnIntegrationPoints(KM.KratosGlobals.GetVariable("SUBSCALE_VELOCITY"), root_model_part.ProcessInfo),
                dtype=float,
            )
            elem_subscale_pressure = np.asarray(
                elem.CalculateOnIntegrationPoints(KM.KratosGlobals.GetVariable("SUBSCALE_PRESSURE"), root_model_part.ProcessInfo),
                dtype=float,
            )
            elem_q_count = int(elem_subscale_velocity.shape[0]) if elem_subscale_velocity.ndim >= 2 else int(elem_subscale_pressure.shape[0])
            subscale_q_counts[idx] = elem_q_count
            if elem_q_count > 0:
                subscale_velocity_blocks.append(np.asarray(elem_subscale_velocity[:, :2], dtype=float))
                subscale_pressure_blocks.append(np.asarray(elem_subscale_pressure, dtype=float).reshape(elem_q_count))
        subscale_velocity_flat = (
            np.concatenate(subscale_velocity_blocks, axis=0)
            if subscale_velocity_blocks
            else np.zeros((0, 2), dtype=float)
        )
        subscale_pressure_flat = (
            np.concatenate(subscale_pressure_blocks, axis=0)
            if subscale_pressure_blocks
        ...
        np.savez(
            ...
            advproj=advproj,
            divproj=divproj,
            fluid_element_ids=element_ids,
            fluid_subscale_q_counts=subscale_q_counts,
            fluid_subscale_velocity_flat=subscale_velocity_flat,
            fluid_subscale_pressure_flat=subscale_pressure_flat,
        ...
        summary = {
            ...
            "subscale_q_count_min": int(np.min(subscale_q_counts)) if subscale_q_counts.size else 0,
            "subscale_q_count_max": int(np.max(subscale_q_counts)) if subscale_q_counts.size else 0,
            "subscale_velocity_inf_norm": float(np.max(np.abs(subscale_velocity_flat))) if subscale_velocity_flat.size else 0.0,
            "subscale_pressure_inf_norm": float(np.max(np.abs(subscale_pressure_flat))) if subscale_pressure_flat.size else 0.0,
        }
