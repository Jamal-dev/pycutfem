...
def main() -> None:
    ...
    with np.load(kratos_state_path) as data:
        ...
        node_coords = np.asarray(data["node_coords_ref"] if "node_coords_ref" in data.files else data["coords_ref"], dtype=float)
        velocity = np.asarray(data["velocity"] if "velocity" in data.files else data["velocity_nodal_values"], dtype=float)
        pressure = np.asarray(
            data["pressure"] if "pressure" in data.files else data["pressure_nodal_values"],
            dtype=float,
        ).reshape(-1, 1)
        ...
        if acceleration is None and "acceleration_nodal_values" in data.files:
            acceleration = np.asarray(data["acceleration_nodal_values"], dtype=float)
        if mesh_displacement is None and "mesh_displacement_nodal_values" in data.files:
            mesh_displacement = np.asarray(data["mesh_displacement_nodal_values"], dtype=float)
        if mesh_velocity is None and "mesh_velocity_nodal_values" in data.files:
            mesh_velocity = np.asarray(data["mesh_velocity_nodal_values"], dtype=float)
