...
class FormCompiler:
    def __init__(self, mesh,function_space, quad_order=None):
        ...
        self.poly_order = mesh.poly_order
    ...
    def visit_Inner(self, node):
        ...
        if self.context['is_rhs']: return np.dot(val_a, val_b)
        return np.outer(val_a, val_b) if val_a.ndim == 1 and val_b.ndim == 1 else val_a @ val_b.T
    ...
    def assemble(self, equation):
        ref = get_reference(self.mesh.element_type, self.poly_order)
        n_loc = len(ref.shape(0,0))
        n_elems = len(self.mesh.elements_list)
        n_dofs = n_loc * n_elems
